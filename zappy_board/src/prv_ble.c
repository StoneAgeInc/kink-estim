//
// Created by Benjamin Riggs on 6/7/20.
//
...
#define BLE_DEVICE_NAME                 DEVICE_NAME                                 /**< Name of device. Will be included in the advertising data. */
...
//            APP_ERROR_CHECK(err);
        ...
        case BLE_GAP_EVT_SEC_PARAMS_REQUEST:
            ...
            err = sd_ble_gap_sec_params_reply(m_conn_handle, BLE_GAP_SEC_STATUS_PAIRING_NOT_SUPP, NULL, NULL);
            APP_ERROR_CHECK(err);
            break;
...
static void nus_data_handler(ble_nus_evt_t * p_evt) {
    ...
    if (p_evt->type == BLE_NUS_EVT_RX_DATA)
    {
        ...
        // Echo bytes back
        err = ble_nus_data_send(&m_nus, (uint8_t *) p_evt->params.rx_data.p_data, &p_evt->params.rx_data.length, m_conn_handle);
        APP_ERROR_CHECK(err);
    }
}
...
//            err = bsp_indication_set(BSP_INDICATE_ADVERTISING);
//            APP_ERROR_CHECK(err);
            break;
...
static void on_conn_params_evt(ble_conn_params_evt_t * p_evt)
{
    ...
    {
        err = sd_ble_gap_disconnect(m_conn_handle, BLE_HCI_CONN_INTERVAL_UNACCEPTABLE);
        APP_ERROR_CHECK(err);
    }
}
...
void ble_init(void) {
    ...
    err = ble_advertising_start(&m_advertising, BLE_ADV_MODE_FAST);
    APP_ERROR_CHECK(err);
}
